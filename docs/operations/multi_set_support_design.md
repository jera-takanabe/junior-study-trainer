...
## Phase 6-3c 実装メモ: 選択中教材 lookup helper の追加

### 完了済み

選択中教材IDから、manifest 上の教材情報を取得するための補助関数を追加した。

追加した関数：

- `getQuestionSetsManifest()`
- `getSelectedQuestionSetItem()`

### 実装内容

`app/index.html` に、登録教材一覧と選択中教材を扱うための helper を追加した。

`getQuestionSetsManifest()` は、`window.QUESTION_SETS_MANIFEST` が配列として存在する場合はそれを返し、存在しない場合は空配列を返す。

`getSelectedQuestionSetItem()` は、`loadSelectedQuestionSetId()` で保存済みの選択中教材IDを取得し、そのIDに一致する教材情報を manifest から探して返す。

選択中教材IDが未保存の場合、または一致する教材がない場合は `null` を返す。
...
- 教材選択ボタンがこれまで通り動く
- クイズ開始ができる
- Console で `typeof getQuestionSetsManifest` が `"function"` になる
- Console で `typeof getSelectedQuestionSetItem` が `"function"` になる
- Console で `getQuestionSetsManifest().length` が `3` になる
- Console で `getSelectedQuestionSetItem()` が選択中教材オブジェクトを返す
...
## Phase 6-4 実装メモ: 起動時の選択教材自動読み込み

### 実装内容

従来の `initialize()` の即時実行を `startApplication()` に置き換え、以下の順序で起動するようにした。

1. `loadSelectedQuestionSetData()` で選択中教材を読み込む
2. 教材データ状態を再構成する
3. `rebuildStorageKeys()` で教材別の保存キーを再構成する
4. `initialize()` で画面を初期化する

教材JSの読み込みに失敗した場合は、既存データから状態を再構成して初期化を続行する。

### 保存キーの対応

`HISTORY_KEY` と `PROGRESS_KEY` を再代入可能にし、`rebuildStorageKeys()` を追加した。

これにより、読み込んだ教材の `questionSetId` に応じて、履歴と進捗の保存先が切り替わる。

### 動作確認

社会地理を選択した状態でページを読み込み、以下を確認した。

- 教材：社会 地理 p10-p53
- 問題数：347問
- セット数：3
- 動的教材JS script 数：1
- 保存キー末尾：`school_social_geography_jhs1_textbook_p10_p53`

続いて社会歴史を選択してページを再読み込みし、以下を確認した。

- 教材：社会 歴史 p24-p27
- 問題数：97問
- セット数：1
- 保存キー末尾：`school_social_history_jhs1_textbook_p24_p27`
- クイズで歴史問題が出題された

### 確認できたこと

選択中教材をアプリ起動時に自動読み込みし、その教材専用の履歴・進捗を使用して初期化できる。

## Phase 6-5 実装メモ: 再読み込み不要の教材切り替え

### 実装内容

教材選択ボタンの処理を非同期化し、`switchQuestionSet(questionSetId)` を追加した。
...
4. 履歴・進捗の保存キーを再構成する
5. 問題IDを検証する
6. セット・カテゴリ・レベルの選択欄を再生成する
7. 問題数・進捗・教材情報・登録教材一覧・保存状態を再表示する

`initialize()` は再実行しない。これにより、画面イベントの重複登録を避けている。

現在読み込み中の教材を再選択した場合は、再読み込み処理を行わない。

教材の読み込みに失敗した場合は、選択中教材IDを以前の値へ戻し、エラーを表示する。

### 画面表示

従来の注意書きを、以下へ変更した。

- `※教材を選択すると、出題教材が自動で切り替わります。`

### 動作確認

ページを再読み込みせず、以下の順に教材を切り替えた。

- 社会 歴史 p24-p27：97問
- 社会 地理 p10-p53：347問
- 理科 中1 教科書 S001-S003：222問
...
- 現在読み込み中と選択中が一致する
- セット一覧が切り替わる
- 履歴・進捗の保存キーが教材IDに合わせて切り替わる
- クイズで選択教材の問題が出題される
- 複数回切り替えても動的教材JS script 数は1のまま

### 現時点の状態

登録済みの3教材は、教材選択ボタンからページ再読み込みなしで切り替えられる。

履歴と進捗は教材ごとに分離して管理される。

Phase 6-3c以前に記載した「実際の教材切り替えは未実装」という状態は、本Phaseの実装により解消した。
